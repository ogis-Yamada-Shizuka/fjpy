class InspectionSchedulesController < ApplicationController
  ...
  def index
    ...
    @inspection_schedules = my_schedules.order_by_target_datetime.page(params[:page])
  end

  # GET /inspection_schedules/1
  # GET /inspection_schedules/1.json
  def show
    @same_place_inspection_schedules = InspectionSchedule.with_place(@inspection_schedule.place).order_by_target_datetime
  end
  ...
  def inspection_schedule_params
    params.require(:inspection_schedule).permit(
      :target_datetime, :equipment_id, :status_id, :service_id, :result_status_id, :processingdate
    )
  end
  ...
  def my_schedules
    ...
    if current_user.head_employee?
      return @search.result.order_by_target_datetime
    end
