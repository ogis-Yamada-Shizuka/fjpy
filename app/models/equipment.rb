class Equipment < ActiveRecord::Base
  ...
  def is_inspection_yearmonth(target_year, target_month)
    ...
    # 点検予定の最未来のものを取る(１件だけ)
    inspection_schedules = self.inspection_schedules.order(targetyearmonth: :desc).limit(1)

    inspection_schedules.each do | last_inspection_schedule |
      # 最未来の点検予定の年月を年と月に分割
      last_ym = last_inspection_schedule.targetyearmonth.scan(/.{1,#{4}}/)
  ...
  def exist_inspection(target_year, target_month)
    if InspectionSchedule.where(equipment_id: self.id, targetyearmonth: target_year+target_month).count == 0
      return false # ない
    else
      return true # ある
    end
